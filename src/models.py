...
class Security:
    ...
    def plot_prices(self):
        ...
        if self.prices is None:
            print(f"❌ Pas de données de prix pour {self.name}")
            return
...
class Portfolio:
    ...
    def plot_boxplots(self):
        ...
        pr = [v['portfolio-total-return'] * 100 for v in self.simulation_results.values()]
        pv = [v['portfolio-std'] * 100 for v in self.simulation_results.values()]
        cvar = [v['portfolio-cvar'] * 100 for v in self.simulation_results.values()]
        mdd = [v['portfolio-mdd'] * 100 for v in self.simulation_results.values()]
    ...
    def plot_portfolio_simulations(self):
        ...
        title = f"Croissance de ${self.portfolio_value:,.0f}: (CAGR:{round(self.cagr_mean*100, 2)}%, Volatilité:{round(self.mean_volatility*100, 2)}%)<br>Nombre de simulations:{sims:,.0f}"

        hover_temp = "<b>Simulation:</b>%{x}<br><b>Valeur du portefeuille:</b>${y:,.0f}<extra></extra>"
        
        fig = go.Figure()
        for sim_id, v in self.simulation_results.items():
            fig.add_trace(
                go.Scatter(
        ...
        fig.update_layout(
            ...
            xaxis_title=FREQUENCY_LABELS[self.simulation_results[0]['frequency']],
            width=700,
            height=500,
            showlegend=False
        )
    ...
    def plot_expected_values(self):
        ...
        if self.expected_values is None or self.simulation_results is None:
            print("❌ Pas de résultats de simulation. Exécutez run_simulation() d'abord.")
            return
        ...
        freq = FREQUENCY_LABELS[self.simulation_results[0]['frequency']]
        periods = len(self.simulation_results[0]['portfolio-values']) - 1
        title_1 = f"Croissance attendue d'un portefeuille de ${self.portfolio_value:,.0f} après {periods} {freq}<br>"
        title_2 = f"Valeur finale: ${lower:,.0f} à ${upper:,.0f} (Confiance 68%)<br>"
        title_3 = f"Sharpe Ratio: {self.sharpe_ratio:.2f} | CAGR: {self.cagr_mean*100:.2f}%"
    ...
    def plot_efficient_frontier(self):
        ...
        secs = "|".join([sec.identifier for sec in self.securities])
